...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void lnk_login_Click_Click(object sender, EventArgs e)
    {
        if (txt_emailid.Text.Length > 0 && txt_password.Text.Length > 0)
        {
            if (dd_login.Text == "Principal")
            {

                st = "select * from principal where Email='" + txt_emailid.Text + "' and Password='" + txt_password.Text + "'";
                dt = get_datatable(st);
                Session["principal"] = txt_emailid.Text;
                if (dt.Rows.Count > 0)
                {
                    int session_princi = dt.Rows[0]["principal_id"].GetHashCode();
                    Session["session_princi"] = session_princi;
                    Session["principal"] = dt.Rows[0][0].ToString();
                    Principal_details obj = new Principal_details();

                    obj.name = dt.Rows[0]["Name"].ToString();
                    Session["principal"] = obj;
            ...
            else if (dd_login.Text == "Placement")
            {

                st = "select name from placement where email='" + txt_emailid.Text + "' and Password='" + txt_password.Text + "'";
                dt = get_datatable(st);
                Session["Placement"] = txt_emailid.Text;
                if (dt.Rows.Count > 0)
                {
                    Session["Placement"] = dt.Rows[0][0].ToString();
                    Placement_details obj = new Placement_details();

                    obj.name = dt.Rows[0]["Name"].ToString();
                    Session["Placement"] = obj;
                    Response.Redirect("Placement/dashboard.aspx");
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "", "showalert('error','Login failed');", true);
            ...
            else if (dd_login.Text == "Student")
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    int session_std = dt.Rows[0]["student_id"].GetHashCode();
